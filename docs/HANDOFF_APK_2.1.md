...
| Current APK | 2.1.2 di produksi, `versionCode 19`; 2.2.0 (`versionCode 20`) siap dirilis dari cabang ini, lihat bagian 11. Sebelumnya 2.1.1, `versionCode 18`, at `/downloads/sakuin.apk` (2.1.0 was live briefly with a broken widget, see section 8), signed with key SHA-1 `A7:20:43:6E:66:BC:64:F1:57:4A:5B:EA:B6:7C:E9:03:4A:31:6D:3A` |
...
- **Notifications** `lib/saku-notifications.ts`:
  ...
  - Transaction reminders use local notification ids 101-106 for 09.00, 13.00, 17.00, then a 21.00-23.00 countdown; the test notification remains id 999. They use `smallIcon: "ic_stat_saku"`, `largeIcon: "saku_notif_happy"`, colour `#2B63E0`.
...
| 7 | Pemilik update ke 2.1.1 di HP Xiaomi, lepas lalu pasang ulang widget, dan memastikan widget tampil. | Selesai: widget 2.1.1 tampil, tetapi layoutnya berantakan menurut pemilik, lalu diperbaiki di 2.1.2. |
| 8 | Rilis APK 2.1.2 (kode 19): widget digambar oleh aplikasi (bagian 10). | Selesai (19 Sep). PR #4 digabung pemilik. Pemilik sudah memeriksa sendiri di HP: widget sudah bagus. |
| 9 | APK 2.2.0 (kode 20): update sekali ketuk, seluruhnya di aplikasi (bagian 11). | Kode, tes emulator, dan APK bertanda tangan selesai (20 Sep). Menunggu pemilik membuka lalu menggabungkan PR, dan memasang 2.2.0 sekali secara manual. |
...
**Rencana teknis** (Tahap A dibatalkan pemilik 20 Sep; Tahap B sudah dikerjakan, lihat bagian 11):
...
## 11. APK 2.2.0: update sekali ketuk, seluruhnya di aplikasi (20 September 2026)

- Keputusan pemilik (20 Sep): **Tahap A di bagian 9 dibatalkan.** Notifikasi update tidak dibuat di sisi web; semuanya ditaruh di aplikasi Android. Jadi tawaran update baru berjalan sendiri setelah pengguna memasang APK 2.2.0 sekali secara manual.
- Yang dibuat:
  - `SakuUpdate.java` — alarm harian (`setAndAllowWhileIdle`, dipasang ulang saat aplikasi dibuka, sesudah boot dan sesudah update), baca `{api}/api/app-version`, bandingkan dengan `versionCode` terpasang, notifikasi "Update Sakuin X sudah ada" + tombol **Perbarui** (satu kali per versi, jadi update yang ditolak tidak ditawarkan tiap hari), unduh lewat DownloadManager ke `getExternalFilesDir(Download)/sakuin-update.apk`, `progress()` 0–100, lalu buka layar pasang Android lewat FileProvider.
  - `SakuUpdateReceiver.java` — menerima alarm harian, `BOOT_COMPLETED`, `MY_PACKAGE_REPLACED`, ketukan tombol "Perbarui", dan `DOWNLOAD_COMPLETE` (→ layar pasang). Receiver sengaja `exported="false"`, jadi tidak bisa dipicu dari `adb am broadcast` — ujinya harus lewat tes instrumentasi.
  - `SakuNotifications.action(...)` (tombol aksi pada notifikasi) + saluran notifikasi `sakuin_update`; sakelar preferensi ke-4 `"update"`; izin `REQUEST_INSTALL_PACKAGES`; `file_paths.xml` menambah `Download/`.
- Batas Android yang tetap berlaku: aplikasi di luar Play Store tidak bisa memasang dirinya diam-diam. Pertama kali, Android meminta izin "Instal aplikasi tidak dikenal" untuk Sakuin, dan layar pasang selalu perlu satu ketukan pemilik.
- Tes baru `apps/web/android/app/src/androidTest/java/com/sakuin/app/SakuUpdateTest.java`: server HTTP palsu di dalam tes (pakai `ServerSocket` biasa — `com.sun.net.httpserver` tidak ada di Android) menyajikan `/api/app-version` versi 9.9.9 dan APK palsu 64 KB. Memastikan notifikasi id 5001 muncul dengan judul dan tombol yang benar, versi yang sama tidak diumumkan dua kali, unduhan selesai 100% dan ukuran berkas utuh.
- Hasil uji di emulator Android 14 (AVD `Sakuin_Widget_34`): `SakuUpdateTest` 1/1 lolos, `SakuNativeTest` 8/8 lolos (perlu `settings put secure show_ime_with_hard_keyboard 1`), unit test JVM lolos. Tes contoh bawaan Capacitor (`com/getcapacitor/myapp/ExampleInstrumentedTest.java`, memeriksa nama paket `com.getcapacitor.app`) dihapus karena sisa template yang selalu gagal.
- Web: typecheck lolos; tes web lolos dengan `npx vitest run --maxWorkers=2` (kalau dijalankan penuh paralel di komputer ini, banyak berkas gagal karena kehabisan sumber daya, bukan karena kode).
- APK 2.2.0: `versionCode 20`, SHA-256 `b7fa651f61e2eedbf2b7549f859f7d6786a2924169051384dd7d53fb24708fa6`, kunci sama seperti rilis sebelumnya (SHA-256 sertifikat `9122e955...`), sudah disalin ke `apps/web/public/downloads/sakuin.apk`. Catatan rilis, `latest-version.json`, `app-version.ts`, `release-notes.ts`, `ApkAppCard`, `LandingPage` (`?v=20`) sudah ikut diperbarui.
- Belum: pemilik membuka dan menggabungkan PR dari cabang ini (push langsung ke `main` diblokir pengaman izin Claude), lalu memasang 2.2.0 sekali secara manual. Setelah itu, update berikutnya cukup satu ketukan dari notifikasi.
