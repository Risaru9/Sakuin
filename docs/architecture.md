...
- Interim screens: `/laporan` reuses the old insight cards and `/lainnya/rekening` the old account card until their redesigned versions land.
- Preview: in development, `/dev/saku` renders every component, and `/dev/beranda` plus `/dev/cari` run the real Beranda and search screens against an in-memory API (`features/dev/dev-fake-api.ts`; add `?kosong=1`, `?lambat=1` or `?gagal=1` on first load). These routes and their chunks are excluded from production builds.
