// The API keeps an identical copy at apps/api/src/modules/transactions/quick-transaction-parser.ts
// so the Android quick-entry window can send plain text. Edit both; a web test compares them.
// The file has no imports so both copies stay byte-for-byte equal.

type TransactionType = "INCOME" | "EXPENSE";

/** The parser only reads these fields, so any category list fits. */
type Category = {
  id: string;
  name: string;
